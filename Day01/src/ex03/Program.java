...
public class Program {
    public static void main(String[] args){
        ...
        for (int i = 0; i < idx.length; i++){
            ...
            newTransaction.setTransferCategory((i % 2 == 0) ? Transaction.transfer_category.debits : Transaction.transfer_category.credits);
            ...
            for (int j = 0; j < array.length; j++){
                ...
                check(tr.getTransferCategory() == ((j % 2 == 0) ? Transaction.transfer_category.debits : Transaction.transfer_category.credits));
            }
        }

        try{
            list.removeTransaction(nonExisting);
            check(false);
        ...
        list.removeTransaction(idx[1]);
        list.removeTransaction(idx[3]);
        check(list.toArray().length == 0);
    }
